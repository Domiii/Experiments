...
{ 
    public class GridCreator : MonoBehaviour
    {
        ...
        {
			if (firstTime || worldMap == null)
			{
				print ("Deleting map: " + cellCount + " cells - " + worldMap);
				...
				current = (GridCreator.CellType[,])worldMap.Clone();
				undoStack.AddLast(new LinkedListNode<GridCreator.CellType[,]>(worldMap));
				//intersectionPoints = new List<Vector3>();

				if (city != null) {
					city.GetComponent<CityPopulator> ().ClearCity ();
				}
            }
            else
            {
