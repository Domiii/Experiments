...
namespace ProceduralCity
{
	public class CityPopulator : MonoBehaviour
	{
		...
		public float streetBuildingMaxDistance = 4;
		public float buildingScale = 0.4f;
		public int treeDensity = 1000;
		...
		public List<List<Vector3>> SampleStreets (List<List<Vector3>> streets)
		{
			...
			for (int i = 0; i < streets.Count; i++) {
				...
				var lastAngle = -1f;
				for (int j = 1; j < streets [i].Count - 1; j++) {
					if (Vector3.Distance (lastAdd, streets [i] [j + 1]) > 0.5f && 
						(Mathf.Abs (lastAngle - Vector3.Angle (lastAdd, streets [i] [j + 1])) > 1f || j == streets[i].Count-1)) {
		...
		public void AddBuildings (GridCreator grid, List<List<Vector3>> streets, float scale)
		{
			...
			for (int i = 0; i < streets.Count; i++) {
				...
				ComputeContourPoints (out side1, out side2, streets [i], streetBuildingMaxDistance);
				...
				for (int j = 0; j < side1.Count - 1; j++) {
					...
					PlaceBuilding (grid, scale, pointFrom, pointTo, ref buildingIndex, streets [i] [j], streets [i] [j + 1], buildings);
