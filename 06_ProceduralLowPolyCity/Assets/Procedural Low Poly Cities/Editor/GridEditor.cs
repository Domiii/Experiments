...
namespace ProceduralCity
{
	...
	public class GridEditor : Editor
	{
		public string[] toolbarStrings = new string[] {
			"Draw Streets",
			"Draw Residential Zone",
			"Draw Skyscraper Zone",
			"Draw Forests",
			"Draw Cone Forests",
			"Draw Woods"
		};
		...
		public void DrawBasicEditTools ()
		{
			...
			grid.brushSize = EditorGUILayout.IntSlider ("Brush size:", grid.brushSize, 2, 10);
			...
			for (int y = 0; y < grid.brushSize; y++) {
				for (int x = 0; x < grid.brushSize; x++) {
					...
					if (gridPos.x + x >= 0 && gridPos.z + y >= 0 && gridPos.x + x < grid.worldMap.GetLength (0) && gridPos.z + y < grid.worldMap.GetLength (1)) {
						if (grid.worldMap [(int)gridPos.x + x, (int)gridPos.z + y] == GridCreator.CellType.Empty) {
							++grid.cellCount;
						}
