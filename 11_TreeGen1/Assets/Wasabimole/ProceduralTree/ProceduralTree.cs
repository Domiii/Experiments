...
namespace Wasabimole.ProceduralTree
{
	...
	public class ProceduralTree : MonoBehaviour
	{
		...
		public void GenerateTreeMesh ()
		{
			#if UNITY_EDITOR
			if (TryGetPrefabMesh ()) {
				// we can use a shared/cached mesh
				return;
			}
			#endif
		...
		void Branch (Quaternion quaternion, Vector3 position, int lastRingVertexIndex, float radius, float texCoordV)
		{
			...
			// Do we branch?
			if (vertexList.Count + NumberOfSides >= MaxNumVertices || Random.value > BranchProbability)
				return;
		...
		public string GetSampleMaterialsPath()
		{
			if (string.IsNullOrEmpty(pathSampleMaterials))
			{
				// Get materials folder
				pathSampleMaterials = AssetDatabase.GetAssetPath(MonoScript.FromMonoBehaviour(this));
				pathSampleMaterials = System.IO.Path.GetDirectoryName(pathSampleMaterials) + "/SampleMaterials/";
			}
			return pathSampleMaterials;
		}

		public void GetSampleMaterial()
		{
			// Get sample materials
			string[] sampleMaterials = System.IO.Directory.GetFiles(GetSampleMaterialsPath(), "*.mat", System.IO.SearchOption.AllDirectories);
			// Return if none found
			if (sampleMaterials.Length < 1) return; 
			// Assign a random sample material
			Renderer.sharedMaterial = AssetDatabase.LoadAssetAtPath(sampleMaterials[Random.Range(0, sampleMaterials.Length)], typeof(Material)) as Material;
		}
		#endif
		...
		public void RandomizeTree(int seed) {
			Seed = seed;
			MaxNumVertices = 1024 << Random.Range(1, 6);
			NumberOfSides = NumSides[Random.Range(0, NumSides.Length)];
			BaseRadius = Random.Range(0.25f, 4f);
			RadiusStep = Random.Range(0.875f, 0.95f);
			MinimumRadius = Random.Range(0.01f, 0.2f);
			BranchRoundness = Random.value;
			SegmentLength = Random.Range(0.25f, 0.75f);
			Twisting = Random.Range(0f, 40f);
			BranchProbability = Random.Range(0f, 0.25f);
		...
		public void Update ()
		{
			...
			GenerateTreeMesh (); // get or generate mesh
